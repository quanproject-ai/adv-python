...
def main():
    ...
    # tint blue square
    level2_file_path = config.PROJECT_DIR + \
        '/video/pyramid/level_002/x_160_y_180_width_160_height_090.mp4'
    name = 'blue'
    r, g, b = 0, 0, 255
    level2_tint = video.color_tint(name, level2_file_path, r, g, b)
    ...
    # level2 square over video, panned the square into it's proper position
    name = 'leve2_over'
    x = 270
    y = 90
    result_video = video.over(
        name, level2_tint, level1_over_level0, x, y)

    # add audio back into file
    name = 'result'
    result_video_with_audio = video.audio(name, result_video,
